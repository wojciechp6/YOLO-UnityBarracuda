...
namespace NN
{
    public class YOLOHandler : IDisposable
    {
        ...
        public YOLOHandler(NNHandler nn)
        {
            ...
            inputWidthHeight = nn.model.inputs[0].shape[5];
        ...
        Tensor Preprocess(Tensor x)
        {
            ...
            var opTensor = new Tensor[] { x, premulTensor };
            var preprocessed = ops.Mul(opTensor);
            Profiler.EndSample();
            return preprocessed;
        }
        ...
        private float[] Softmax(float[] values)
        {
            ...
            var ret = ops.Softmax(t, axis: -1).AsFloats();
            t.Dispose();
            return ret;
        }
